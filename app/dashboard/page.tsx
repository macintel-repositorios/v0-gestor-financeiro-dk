...
// Componente de Loading Skeleton adaptado ao novo padrão minimalista e elegante
function DashboardSkeleton() {
  return (
    <div className="flex-1 space-y-6 p-6 md:p-10 w-full bg-transparent text-foreground relative overflow-hidden">
      {/* Light blobs for loading state */}
      <div className="absolute top-[-10%] right-[-5%] w-[400px] h-[400px] rounded-full bg-primary/5 blur-[100px] pointer-events-none -z-10 animate-pulse" />
      <div className="absolute bottom-[-10%] left-[-5%] w-[500px] h-[500px] rounded-full bg-indigo-500/5 blur-[120px] pointer-events-none -z-10 animate-pulse" />
    ...
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto w-full text-foreground relative overflow-hidden">
          ...
          <div>
            ...
            <div className="mt-4">
              <Button
                ...
                className="flex items-center justify-center gap-2 h-10 rounded-xl border border-border bg-card hover:bg-slate-100/10 hover:border-indigo-500/30 shadow-xs px-4 text-xs font-semibold transition-all duration-200"
              >
                {!shouldShow ? (
                  <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
        ...
        <Card
          ...
          className="group relative cursor-pointer hover:border-indigo-500/50 hover:bg-muted/10 transition-all duration-300 border border-border/80 rounded-xl shadow-xs bg-card overflow-hidden hover:ring-2 hover:ring-indigo-500/30"
        >
          <div className="absolute top-0 left-0 w-full h-[3px] bg-indigo-500" />
          <CardHeader className="flex flex-row items-center justify-between space-y-0 p-6 pb-2">
            <CardTitle className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider">Total de Clientes</CardTitle>
            <Users className="h-5 w-5 text-indigo-500 group-hover:scale-110 transition-transform" />
          </CardHeader>
          <CardContent className="p-6 pt-0">
            <div className="text-2xl font-bold tracking-tight">{stats.totalClientes}</div>
        ...
        <Card
          ...
          className="group relative cursor-pointer hover:border-emerald-500/50 hover:bg-muted/10 transition-all duration-300 border border-border/80 rounded-xl shadow-xs bg-card overflow-hidden hover:ring-2 hover:ring-emerald-500/30"
        >
          <div className="absolute top-0 left-0 w-full h-[3px] bg-emerald-500" />
          <CardHeader className="flex flex-row items-center justify-between space-y-0 p-6 pb-2">
            <CardTitle className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Receita em Boletos</CardTitle>
            <DollarSign className="h-5 w-5 text-emerald-500 group-hover:scale-110 transition-transform" />
          </CardHeader>
          <CardContent className="p-6 pt-0">
            <div className="text-2xl font-bold tracking-tight text-emerald-600 dark:text-emerald-400">{formatValueOrHide(stats.valorTotalBoletos)}</div>
            <p className="text-xs text-muted-foreground mt-1">
        ...
        <Card
          ...
          className="group relative cursor-pointer hover:border-blue-500/50 hover:bg-muted/10 transition-all duration-300 border border-border/80 rounded-xl shadow-xs bg-card overflow-hidden hover:ring-2 hover:ring-blue-500/30"
        >
          <div className="absolute top-0 left-0 w-full h-[3px] bg-blue-500" />
          <CardHeader className="flex flex-row items-center justify-between space-y-0 p-6 pb-2">
            <CardTitle className="text-xs font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wider">Total em Orçamentos</CardTitle>
            <Calculator className="h-5 w-5 text-blue-500 group-hover:scale-110 transition-transform" />
          </CardHeader>
          <CardContent className="p-6 pt-0">
            <div className="text-2xl font-bold tracking-tight text-blue-600 dark:text-blue-400">{formatValueOrHide(stats.valorTotalOrcamentos)}</div>
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground mt-1">
              <span className={cn(
                "font-semibold flex items-center",
                taxaConversao >= 50 ? "text-emerald-400" : "text-amber-400"
              )}>
        ...
        <Card
          ...
          className={cn(
            "group relative cursor-pointer transition-all duration-300 border rounded-xl shadow-xs bg-card overflow-hidden hover:ring-2 hover:ring-rose-500/30",
            stats.boletosVencidos > 0 
              ? "border-rose-500/50 hover:border-rose-500 hover:bg-rose-950/10" 
              : "border-border/80 hover:border-rose-500/30 hover:bg-muted/10"
          )}
        >
          <div className="absolute top-0 left-0 w-full h-[3px] bg-rose-500" />
          <CardHeader className="flex flex-row items-center justify-between space-y-0 p-6 pb-2">
            <CardTitle className="text-xs font-semibold text-rose-600 dark:text-rose-400 uppercase tracking-wider">Alertas Ativos</CardTitle>
            <AlertTriangle className={cn("h-5 w-5 group-hover:scale-110 transition-transform", stats.boletosVencidos > 0 ? "text-rose-500" : "text-rose-500/80")} />
          </CardHeader>
          <CardContent className="p-6 pt-0">
            <div className={cn("text-2xl font-bold tracking-tight", stats.boletosVencidos > 0 ? "text-rose-500" : "text-rose-600 dark:text-rose-400")}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start w-full">
        ...
        <Card className="lg:col-span-2 border border-border/80 shadow-md rounded-xl bg-card overflow-hidden hover:border-indigo-500/20 transition-all duration-300">
          <CardHeader className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-t-lg p-4 lg:p-6 dark:from-blue-900/50 dark:to-indigo-900/50 dark:border-b dark:border-border">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <CardTitle className="text-white text-base lg:text-lg font-bold">Atividade Recente</CardTitle>
                <CardDescription className="text-blue-100 text-xs">Últimos boletos e orçamentos emitidos no sistema</CardDescription>
              </div>
              ...
              <div className="relative w-full sm:w-60">
                ...
                <Input
                  ...
                  className="pl-9 h-8.5 text-xs bg-background/90 text-foreground border-border/40 focus:border-white focus:ring-1 focus:ring-white rounded-xl"
                />
              </div>
            </div>
          </CardHeader>
                ...
                <table className="w-full border-collapse text-left">
                  <thead>
                    {table.getHeaderGroups().map((headerGroup) => (
                      <tr key={headerGroup.id} className="border-b border-border/40 bg-slate-50/30 dark:bg-slate-950/40">
                  ...
                  <tbody className="divide-y divide-border/40">
                    {table.getRowModel().rows.map((row) => (
                      <tr key={row.id} className="hover:bg-slate-50/20 dark:hover:bg-slate-900/20 transition-colors">
              ...
              <div className="p-4 border-t border-border/40 flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    className="h-8 px-2 text-xs border-border bg-card rounded-lg hover:border-indigo-500/30"
                  >
        ...
        {/* Quick Actions Panel */}
        <Card className="border border-border/80 shadow-md rounded-xl bg-card overflow-hidden hover:border-indigo-500/20 transition-all duration-300">
          <CardHeader className="bg-gradient-to-r from-purple-500 to-indigo-600 text-white rounded-t-lg p-4 lg:p-6 dark:from-purple-900/50 dark:to-indigo-900/50 dark:border-b dark:border-border">
            <CardTitle className="text-white text-base lg:text-lg font-bold">Ações Rápidas</CardTitle>
            <CardDescription className="text-purple-100 text-xs">Inicie cadastros ou abra novos formulários diretamente</CardDescription>
          </CardHeader>
          <CardContent className="space-y-2 p-6">
            ...
            <Button
              ...
              className="w-full justify-start h-11 hover:bg-indigo-600/10 hover:text-indigo-400 rounded-xl group border border-transparent hover:border-indigo-500/20 transition-all duration-200 text-xs font-semibold"
            >
              <div className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-900/60 mr-3 border border-border group-hover:bg-indigo-500/20 transition-colors">
                <Users className="h-3.5 w-3.5 text-slate-600 dark:text-slate-400 group-hover:text-indigo-400" />
              </div>
            ...
            >
              <div className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-900/60 mr-3 border border-border group-hover:bg-indigo-500/20 transition-colors">
                <Package className="h-3.5 w-3.5 text-slate-600 dark:text-slate-400 group-hover:text-indigo-400" />
              </div>
            ...
            >
              <div className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-900/60 mr-3 border border-border group-hover:bg-indigo-500/20 transition-colors">
                <DollarSign className="h-3.5 w-3.5 text-slate-600 dark:text-slate-400 group-hover:text-indigo-400" />
              </div>
              Emitir Novo Boleto
            </Button>
            ...
            >
              <Link href="/ordem-servico?nova=true">
                <div className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-900/60 mr-3 border border-border group-hover:bg-indigo-500/20 transition-colors">
                  <Wrench className="h-3.5 w-3.5 text-slate-600 dark:text-slate-400 group-hover:text-indigo-400" />
                </div>
            ...
            >
              <Link href="/orcamentos/novo">
                <div className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-900/60 mr-3 border border-border group-hover:bg-indigo-500/20 transition-colors">
                  <Calculator className="h-3.5 w-3.5 text-slate-600 dark:text-slate-400 group-hover:text-indigo-400" />
                </div>
                Novo Orçamento
              </Link>
            </Button>
            ...
            >
              <Link href="/contratos/proposta/nova">
                <div className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-900/60 mr-3 border border-border group-hover:bg-indigo-500/20 transition-colors">
                  <FileContract className="h-3.5 w-3.5 text-slate-600 dark:text-slate-400 group-hover:text-indigo-400" />
                </div>
            ...
            <div className="mt-4 p-4 bg-muted/30 border border-border/60 rounded-xl">
              ...
              <div className="space-y-2 text-[10px] font-medium text-muted-foreground">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline" className="bg-emerald-950/20 text-emerald-400 border-emerald-900/30 text-[9px] py-0 px-2 rounded-full font-semibold">
                    Estável
                  </Badge>
                </div>
                <div className="flex items-center justify-between">
      ...
      {(stats.boletosVencidos > 0 || stats.boletosPendentes > 5) && (
        <Card className="border border-rose-500/30 bg-rose-500/5 rounded-xl shadow-xs overflow-hidden w-full">
          <CardContent className="p-6">
            <div className="flex items-start gap-4">
              <div className="p-2 rounded-lg bg-rose-950/40 text-rose-400 border border-rose-900/30 shrink-0">
                <AlertTriangle className="h-5 w-5" />
              </div>
              <div className="flex-1">
                <h3 className="font-semibold text-rose-300 text-sm">Pendências Financeiras</h3>
                <p className="text-xs text-rose-400/80 mt-1">
                  {stats.boletosVencidos > 0 && (
                    <span>
                      Você possui <strong className="text-rose-200 font-bold">{stats.boletosVencidos} boleto{stats.boletosVencidos > 1 ? "s" : ""} vencido{stats.boletosVencidos > 1 ? "s" : ""}</strong>
                      {stats.boletosPendentes > 5 && " e "}
                    </span>
                  )}
                  {stats.boletosPendentes > 5 && (
                    <span>
                      <strong className="text-amber-300 font-bold">{stats.boletosPendentes} boletos pendentes</strong> aguardando pagamento
                    </span>
                  )}
                </p>
                <Button
                  variant="outline"
                  size="sm"
                  className="mt-3 border-rose-900/50 text-rose-400 hover:bg-rose-950 dark:hover:text-rose-300 transition-all font-medium text-xs h-8 bg-transparent"
                  asChild
                >
