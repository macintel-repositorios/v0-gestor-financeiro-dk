...
import { Save, Loader2, Eye, Calculator, Lock, RefreshCw, Package } from "lucide-react"
...
export function NovoProdutoDialog({ open, onOpenChange, onSuccess }: NovoProdutoDialogProps) {
  ...
  // Generate code preview when type or brand changes
  useEffect(() => {
  ...
  }, [formData.tipo, formData.marca, categorias])
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-xl h-full flex flex-col p-6 overflow-y-auto border-l border-border shadow-2xl bg-card text-foreground">
        <SheetHeader className="mb-4">
          ...
          <SheetDescription>Cadastre um novo produto no sistema</SheetDescription>
        </SheetHeader>
              ...
              <div className="flex items-center gap-2 p-3 bg-indigo-500/10 rounded-lg border border-indigo-500/20">
                {generatingCode ? (
                  <>
                    ...
                    <span className="text-xs font-medium text-indigo-600 dark:text-indigo-400">Gerando código...</span>
                  </>
                ) : (
                  <>
                    <Eye className="h-4 w-4 text-indigo-500" />
                    <span className="text-xs font-medium text-indigo-600 dark:text-indigo-400">Código gerado:</span>
                    <span className="font-mono font-bold text-indigo-600 dark:text-indigo-400 text-sm">{previewCodigo}</span>
                  </>
                )}
              </div>
            )}
