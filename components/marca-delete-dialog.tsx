  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          title: "Erro",
          description: result.message || "Erro ao excluir marca",
          variant: "destructive",
        })
      }
    } catch (error) {
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="w-full sm:max-w-md h-full flex flex-col p-6 border-l border-border shadow-2xl bg-card text-foreground">
        <SheetHeader className="mb-6">
          ...
          <SheetDescription>
            Esta ação não pode ser desfeita. A marca será permanentemente removida do sistema.
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="flex-1 space-y-4">
          <div className="bg-muted p-4 rounded-lg space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-foreground">{marca.sigla}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-muted-foreground">Produtos vinculados:</span>
              <Badge variant="outline" className="border-border text-foreground">{marca.total_produtos}</Badge>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-muted-foreground">Status:</span>
              <Badge variant={marca.ativo ? "default" : "secondary"}>{marca.ativo ? "Ativo" : "Inativo"}</Badge>
            </div>
          </div>

          {marca.total_produtos > 0 && (
            <div className="bg-red-500/10 border border-red-500/20 p-3 rounded-lg">
              ...
              <p className="text-xs text-red-500 mt-1">
                Esta marca possui {marca.total_produtos} produto(s) vinculado(s). A exclusão não será permitida.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-3 mt-6 border-t border-border pt-4">
          ...
          <Button
            ...
            disabled={loading || marca.total_produtos > 0}
